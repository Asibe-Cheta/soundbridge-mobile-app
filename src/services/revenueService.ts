...
class RevenueService {
  ...
  // Updated per web team response - subscription data in user_subscriptions table, use API endpoint
  ...
  async getSubscription(userId: string): Promise<SubscriptionData | null> {
    try {
      ...
      // Transform API response to match SubscriptionData interface
      return {
  ...
  async updateSubscription(userId: string, tier: 'pro', billingCycle: 'monthly' | 'yearly'): Promise<{ success: boolean; error?: any }> {
    try {
      ...
      const subscriptionData = {
        ...
        cancel_at_period_end: false,
        amount: amounts[normalizedTier][billingCycle],
        currency: 'USD',
  ...
  async getUsageStats(userId: string): Promise<UsageStats | null> {
    try {
      ...
      const tierLimits = limits[normalizedTier as keyof typeof limits] || limits.free;
...
export const revenueService = new RevenueService();
  ...
  async getUsageStats(userId: string): Promise<UsageStats | null> {
    ...
    try {
      ...
      // Defensive: if tier is 'enterprise', treat as 'pro'
      const normalizedTier = subscription?.tier === 'enterprise' ? 'pro' : (subscription?.tier || 'free');
