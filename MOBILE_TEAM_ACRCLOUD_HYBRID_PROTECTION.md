...
**Request Format:** `multipart/form-data` (recommended) or JSON with base64 (deprecated)

**Multipart Form Fields (Recommended):**
- `audioFile` (File, required) - Audio file to fingerprint (no base64 encoding needed)
...
**Legacy Base64 Request Body (Deprecated - May fail for files > 10 MB):**
```json
{
  "fileData": "data:audio/mpeg;base64,/9j/4AAQSkZJRg...", // DEPRECATED: Use multipart instead
  "artistName": "John Doe"
}
```
...
### Step 2: Call Fingerprint API (Updated - Multipart/Form-Data)

```typescript
const fingerprintAudio = async (file: { uri: string; name: string; type: string }) => {
  setAcrcloudStatus('checking');

  try {
    // Create FormData with audio file (no base64 conversion needed)
    ...
    if (artistName && artistName.trim()) {
      uploadFormData.append('artistName', artistName);
    }

    // Call fingerprint API with multipart/form-data
